...
class rsssl_premium_options {
    ...
    public function auto_update_elementor_url() {
	    if (RSSSL()->really_simple_ssl->ssl_enabled && !get_option('elementor_upgraded') && uses_elementor()){
            ...
            Utils::replace_urls( $from, $to );
            update_option('elementor_upgraded', true);
	    }
    }
    ...
    public function show_nginx_headers_notice() {
        ...
        if( !is_multisite() ) {
            if (RSSSL()->rsssl_server->get_server() === 'nginx' && !get_site_option("rsssl_nginx_message_shown")) {
                ...
                <div id="message" class="notice updated is-dismissible">
                    <p>
                        <?php _e("Really Simple SSL has detected NGINX as webserver. The security headers can be set in two ways: via an nginx.conf file in the websites root directory, or via PHP."); ?>
                        <br>
