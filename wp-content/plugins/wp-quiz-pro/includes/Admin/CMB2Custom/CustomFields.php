...
class CustomFields {
	...
	public function render_aweber( CMB2_Field $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
		...
		echo $field_type_object->title(
			array(
				'name' => __( 'Enter your app ID then click Get auth code button.', 'wp-quiz-pro' ),
			)
		); // WPCS: xss ok.
		echo '<p><input type="text" class="aweber-app-id" placeholder="' . esc_attr__( 'App ID', 'wp-quiz-pro' ) . '"></p>';
		printf(
			'<p><a href="#" target="_blank" class="button button-primary aweber-get-auth-code-button">%s</a></p>',
			esc_html__( 'Get auth code', 'wp-quiz-pro' )
		);
		...
		echo $field_type_object->title(
			array(
				'name' => __( 'Copy and paste the authorization code you see after log in to your AWeber account.', 'wp-quiz-pro' ),
			)
		); // WPCS: xss ok.
		echo '<p><input type="password" class="aweber-auth-code regular-text" placeholder="' . esc_attr__( 'Auth code', 'wp-quiz-pro' ) . '"></p>';
		echo '<p><button type="button" class="button button-primary aweber-auth-button">' . esc_html__( 'Authorize', 'wp-quiz-pro' ) . '</button></p>';
		echo '</div><!-- End .aweber-auth-code-step -->';

		echo '<div class="aweber-list-id-step">';
		$desc = sprintf(
			// translators: disconnect button.
			__( 'Your AWeber Account is connected. %s to disconnect.', 'wp-quiz-pro' ),
			sprintf(
				'<a href="#" class="aweber-disconnect-button">%s</a>',
				__( 'Click here', 'wp-quiz-pro' )
			)
		);
		echo '<p>' . wp_kses_post( $desc ) . '</p>';
		$id = $field_type_object->_id( '_listid' );
		echo '<p><label for="' . esc_attr( $id ) . '"><strong>' . esc_html__( 'List ID', 'wp-quiz-pro' ) . '</strong></label></p>';
		echo '<p>';
		echo $field_type_object->input(
			array(
